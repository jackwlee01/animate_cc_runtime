...
/*
type AnimLogicCallback = (item:Drawable, frame:number, lerp:boolean)=>void
type AnimLogic = (item:Drawable, frame:number, lerp:boolean, callback:AnimLogicCallback)=>void



function composeLogic(a:AnimLogic, b:AnimLogic){
    const callback:AnimLogicCallback = (item, frame, lerp) => {
        b(item, frame, lerp)
    }

    return function(item:Drawable, frame:number, lerp:boolean){
        a(item, frame, lerp, callback)
    }
}

const mouseDrag:AnimLogic = (item, frame, lerp, callback) => {

}

const highlight:AnimLogic = (item, frame, lerp, callback) => {

}


const logic = composeLogic(mouseDrag, highlight)
*/

// Function composition utility types and functions
type AnimLogicCallback = (item:Drawable, frame:number, lerp:boolean)=>void
type AnimLogic = (callback:AnimLogicCallback) => AnimLogicCallback
type StatefulAnimLogic<S> = (state:S) => AnimLogic


const chain = (...fns:Array<AnimLogic>) => {
    const complete:AnimLogicCallback = (item, frame, lerp) => item.draw(frame, lerp)
    let last = complete;
...
// Logic
const mouseDrag:AnimLogic = callback => (item, frame, lerp) => {

}

const highlight:StatefulAnimLogic<{highlight:Drawable|null}> = state => callback => (item, frame, lerp) => {
...
// State
const state = {
    highlight:null as Drawable|null
}
