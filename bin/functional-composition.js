(() => {
  ...
  // src/examples/functional-composition.ts
  ...
  function createState() {
    ...
    return function drawWithLogic(item, frame2, lerp) {
      ...
      if (item instanceof Clip2) {
        const highlight2 = selection && item.name == "StarDude";
        if (highlight2)
  ...
  var chain = (...fns) => {
    const complete = (item, frame2, lerp) => item.draw(frame2, lerp);
    let last = complete;
  ...
  var mouseDrag = (callback) => (item, frame2, lerp) => {
  };
  var highlight = (state2) => (callback) => (item, frame2, lerp) => {
  };
  var state = {
    highlight: null
  };
  chain(mouseDrag, highlight(state));
...
//# sourceMappingURL=functional-composition.js.map
